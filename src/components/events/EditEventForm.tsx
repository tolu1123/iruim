...
import { ChevronDownIcon, Router } from "lucide-react";
...
type EventDataProps = EventFormValues & { id: string; imageUrl: string };
...
export default function EditEventForm({
  ...
  async function onSubmit(data: EventFormValues) {
    ...
    // Check if it is a raw file, if it is, upload and get the url to store
    ...
    const editEventWithId = editEvent.bind(null, eventData.id);
    const res = await editEventWithId(data);
    if (!res.success) {
    ...
    router.push(`/list-events/${eventData.id}/configure-event`);
  }
    ...
    <section className=''>
      <div className='w-full xl:max-w-[1140px] mx-auto px-5 py-5 md:pt-5 md:pb-20'>
        <div className='mb-8'>
          ...
          <p className='text-xl'>Edit this event.</p>
        </div>
